...
> tool-sonucu turu, 15.000 train + 2.000 val + ~330 hard_eval).
...
- **UYGULAMA:** `catalog/catalog.py` (`_compute_disc_kw`, `_SYN_OVERRIDE`),
  `scripts/gen/frames.py` (`USER_FRAMES_OBLIQUE`), `scripts/gen/scenarios.py`
  (`_fill_user` oblique_p=0.72).
- **ÖLÇÜLEN SONUÇ:** ayırt edici yüzey kelimesi → tool korelasyonu **%97 → %35**
  (hedef < %55). `test_keyword_to_toolname_shortcut_is_low` geçiyor.
...
- **ÖLÇÜLEN SONUÇ:** ≤12 %34 · 13–34 %51 · 35–58 %15 (medyan 19, p90 43, maks 58).
  Hedef konumu ort. **0.49** (ele vermiyor). `test_candidate_size_buckets`,
...
- **ÖLÇÜLEN SONUÇ:** benzersiz ilk-kullanıcı-turu **%96.6**. Halüsinasyon 0 —
...
- **ÖLÇÜLEN SONUÇ:** A 450 · F 375 · E 300 · D 300 (n=15000). `test_hard_negatives_present`
  geçiyor. hard_eval'de ayrıca P4 (16) ve P8 (12) probe'ları.
...
- **DEĞİŞİKLİK:** Üç ayrı değerlendirme yüzeyi:
  ...
  - `hard_eval` (~330) — test tool'ları + 9 policy-probe (P1…P9), elle yazılmış
    havuz-dışı sarmallar.
...
| D-4 | Yüzey–ad kısayolunu kır | ✅ | K-1 korelasyonu %35 |
...
| D-10 | Opsiyonel parametre davranışı | ⚠️→✅ | %7.3 örnekte opsiyonel dolduruluyor (Durum A); "verilmezse uydurma" (Durum B) tüm read'lerde; dolaylı verme (Durum C) enum yüzeyleriyle |
...
```
python scripts/validate_dataset.py   -> HATA 0 / UYARI 0  (train 15000 + val 2000 + hard_eval ~330)
